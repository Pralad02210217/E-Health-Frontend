...
import { fetchIllnessCategoryFn } from '@/lib/api';
...
export default function IllnessPage() {
  ...
  const { data: illnessData, isLoading, isError } = useQuery({
    queryKey: ['illnessesCategory'],
    queryFn: fetchIllnessCategoryFn,
  });
  ...
  const illnesses = illnessData?.data?.categories || [];
